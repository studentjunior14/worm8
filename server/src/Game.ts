...
export class Game {
    ...
    public static getInstance(io?: Server): Game {
        if (!Game.instance && io) Game.instance = new Game(io);
    ...
    private loop() {
        ...
        for (let i = Snake.allSnakes.length - 1; i >= 0; i--) {
            ...
            if (!s.isActive) {
                ...
                // Check event flags
                if (s.didHeadshot) {
                    if (s.socketId) this.io.to(s.socketId).emit('headshot');
                    s.didHeadshot = false;
                }
                if (s.didKill) {
                    if (s.socketId) this.io.to(s.socketId).emit('kill');
                    s.didKill = false;
                }
            }
        }
    ...
    public handleInput(socketId: string, data: { angle: number, boosting: boolean }) {
        const snake = Snake.allSnakes.find(s => s.socketId === socketId);
        if (snake) {
            snake.setInput(data);
        }
